    ...
    for epoch in range(100000):
        for mode in ['train', 'val']:
            ...
            if mode == 'train':
                ...
                df = pd.DataFrame(list(zip(*[epochs, iou_scores, L_data1_losses]))).add_prefix('Col')
                filename = path + 'G1_e' + str(epoch) + '_' + time_begin + '.csv'
                ...
                df = pd.DataFrame(list(zip(
                    *[ave_epochs, ave_iou_scores, ave_L_data1_losses]))).add_prefix('Col')
                filename = path + 'G1_ave_e' + str(epoch) + '_' + time_begin + '.csv'
                print('saving to', filename)
                df.to_csv(filename, index=False)
                del(df)
                ...
                # save every 25 iterations points
                df = pd.DataFrame(list(zip(*[val_epochs, val_iou_scores, val_L_data1_losses]))).add_prefix('Col')
                filename = path + 'G1_val_e' + str(epoch) + '_' + time_begin + '.csv'
                print('saving to', filename)
                df.to_csv(filename, index=False)
                ...
                # saves lists of average (per epoch) losses
                df = pd.DataFrame(list(zip(
                    *[val_ave_epochs, val_ave_iou_scores, val_ave_L_data1_losses]))).add_prefix('Col')
                filename = path + 'G1_val_ave_e' + str(epoch) + '_' + time_begin + '.csv'
                print('saving to', filename)
                df.to_csv(filename, index=False)
                del(df)
